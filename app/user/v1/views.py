...
class UserVieSets(viewsets.ModelViewSet):
    ...
    @action(methods=['POST'], detail=False, serializer_class=CreateUserSerializer, url_path='invite-user')
    ...
    def verify_token(self, request, pk=None):
        ...
        if serializer.is_valid():
            ...
            return Response({'success': False, 'valid': False}, status=status.HTTP_404_NOT_FOUND)
        return Response({'success': False, 'errors': serializer.errors}, status.HTTP_400_BAD_REQUEST)

    @action(methods=['POST'], detail=False, serializer_class=InitPasswordResetSerializer,
            url_path='reset-password', permission_classes=[AllowAny])
    def initialize_reset(self, request, pk=None):
        ...
        if serializer.is_valid():
            ...
            if not user:
                return Response({'success': False, 'message': 'user with this record not found'},
                                status=status.HTTP_400_BAD_REQUEST)
            token, created = Token.objects.update_or_create(user=user, token_type='ResetToken', defaults={
                'user': user, 'token': generate_token(user), 'token_type': 'ResetToken'})
            email_data = {'fullname': user.firstname, 'email': user.email, 'token': token.token}
    ...
    @action(methods=['POST'], detail=False, serializer_class=PinSerializer, url_path='verify-pin')
