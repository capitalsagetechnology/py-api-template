...
class CreateUserSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        user = User.objects.create_user(**validated_data, password=get_random_string(10))
        token_str = generate_token(user)
        token, _ = Token.objects.update_or_create(
            user=user, token_type='CreateToken',
            defaults={'user': user, 'token_type': 'CreateToken', 'token': token_str})

        user_data = {'id': user.id, 'email': user.email, 'fullname': "Team Member",
